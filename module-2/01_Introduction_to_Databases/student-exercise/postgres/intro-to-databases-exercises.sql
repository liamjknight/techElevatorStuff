...
SELECT name, continent, gnp AS gross_national_product FROM country WHERE gnp > 1000000;
...
SELECT name, continent, gnp AS gross_national_product, lifeexpectancy AS average_life_expectancy FROM country WHERE lifeexpectancy > 80;
...
SELECT name, continent, headofstate AS head_of_state FROM country WHERE governmentform = 'Monarchy';
...
SELECT name, population, gnp AS gross_national_product FROM country WHERE gnp > 1000000 AND population < 100000000;
...
SELECT name, continent, gnp AS gross_national_product, lifeexpectancy FROM country WHERE lifeexpectancy > 70 AND gnp BETWEEN 1 AND 100;
-- 21. The per capita GNP (i.e. GNP divided by population) in US Dollars of all countries in Europe (46 rows)
SELECT gnp*1000000/population AS per_capita_gnp FROM country where continent = 'Europe';
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT 2020-c.indepyear AS years_since_independence FROM country AS c WHERE indepyear IS NOT NULL;