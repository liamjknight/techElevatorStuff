...
SELECT campground_id, daily_fee FROM campground;
...
SELECT CAST(AVG(visitors) AS NUMERIC(8,0)) FROM park;
...
SELECT * FROM site WHERE utilities = 'Y' ORDER BY max_rv_length DESC;

-- select total number of sites that have utilities hook up
SELECT count(site_id) FROM site WHERE utilities = 'Y';
...
SELECT reservation_id, site_id, name, from_date, to_date FROM reservation WHERE from_date >= CAST('2020-10-01' AS date) AND from_date >= CAST('2020-10-31' AS date);
